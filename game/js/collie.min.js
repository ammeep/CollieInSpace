var collie=collie||{};(function(){collie.version="1.0.6";collie.Class=function(f,e){var c=null;if("$init" in f){c=f.$init;
delete f.$init}var d=function(){if(c!==null){c.apply(this,arguments)}};if(typeof e!="undefined"){d=function(){e.apply(this,arguments);
if(c!==null){c.apply(this,arguments)}};var b=function(){};b.prototype=e.prototype;d.$super=e.prototype;d.prototype=new b();
d.prototype.constructor=d}for(var a in f){if(f.hasOwnProperty(a)&&a!="prototype"){d.prototype[a]=f[a]}}return d};collie.util=new (collie.Class({$init:function(){this._sCSSPrefix=null;
this._htDeviceInfo=null;this._bSupport3d=null;this._bSupportCSS3=null;this._htBoundary={left:0,right:0,top:0,bottom:0}},getDisplayObjectById:function(a){return collie.DisplayObject.htFactory[a]
},getDisplayObjectByName:function(b){for(var a in collie.DisplayObject.htFactory){if(collie.DisplayObject.htFactory[a].get("name")==b){return collie.DisplayObject.htFactory[a]
}}return false},getDeviceInfo:function(){if(this._htDeviceInfo!==null){return this._htDeviceInfo}var c=null;var b=false;var e=typeof CanvasRenderingContext2D!="undefined";
var g=false;var h=false;var d=false;var f=navigator.userAgent;var a=0;if(/android/i.test(f)){g=true;c=f.toString().match(/android ([0-9]\.[0-9])/i);
if(c&&c[1]){a=parseFloat(c[1])}}else{if(/(iphone|ipad|ipod)/i.test(f)){h=true;c=f.toString().match(/([0-9]_[0-9])/i);if(c&&c[1]){a=parseFloat(c[1].replace(/_/,"."))
}}else{b=true;if(/(MSIE)/i.test(f)){d=true;c=f.toString().match(/MSIE ([0-9])/i);if(c&&c[1]){a=parseInt(c[1],10)}}}}this._htDeviceInfo={supportCanvas:e,desktop:b,android:g?a:false,ios:h?a:false,ie:d?a:false};
return this._htDeviceInfo},getCSSPrefix:function(f,b){var d="";if(this._sCSSPrefix===null){this._sCSSPrefix="";if(typeof document.body.style.webkitTransform!=="undefined"){this._sCSSPrefix="-webkit-"
...
}else{if(typeof document.body.style.msTransform!=="undefined"){this._sCSSPrefix="-ms-"}}}}}d=this._sCSSPrefix+(f?f:"");if(b){var e=d.split("-");
d="";for(var c=0,a=e.length;c<a;c++){if(e[c]){d+=d?e[c].substr(0,1).toUpperCase()+e[c].substr(1):e[c]}}if(this._sCSSPrefix=="-moz-"||this._sCSSPrefix=="-o-"){d=d.substr(0,1).toUpperCase()+d.substr(1)
}}return d},getSupportCSS3:function(){if(this._bSupportCSS3===null){this._bSupportCSS3=typeof document.body.style[collie.util.getCSSPrefix("transform",true)]!="undefined"||typeof document.body.style.transform!="undefined"
...
}return this._bSupport3d},toRad:function(a){return a*Math.PI/180},toDeg:function(a){return a*180/Math.PI},approximateValue:function(a){return Math.round(a*10000000)/10000000
},fixAngle:function(a){var b=collie.util.toDeg(a);b-=Math.floor(b/360)*360;return collie.util.toRad(b)},getDistance:function(b,d,a,c){return Math.sqrt(Math.pow(a-b,2)+Math.pow(c-d,2))
},getBoundary:function(b){var g=Number.MAX_VALUE;var f=Number.MAX_VALUE;var e=Number.MIN_VALUE;var d=Number.MIN_VALUE;for(var c=0,a=b.length;
c<a;c++){g=Math.min(g,b[c][0]);e=Math.max(e,b[c][0]);f=Math.min(f,b[c][1]);d=Math.max(d,b[c][1])}this._htBoundary.left=g;
this._htBoundary.right=e;this._htBoundary.top=f;this._htBoundary.bottom=d;return this._htBoundary},getBoundaryToPoints:function(a){return[[a.left,a.top],[a.right,a.top],[a.right,a.bottom],[a.left,a.bottom]]
},queryString:function(){var b={};if(location.search){var e=location.search.substr(1).split("&");for(var c=0,a=e.length;c<a;
c++){var d=e[c].split("=");b[d.shift()]=d.join("=")}}return b},cloneObject:function(a){var c={};for(var b in a){c[b]=a[b]
}return c},pushWithSort:function(e,d){var b=false;for(var c=0,a=e.length;c<a;c++){if(e[c].get("zIndex")>d.get("zIndex")){e.splice(c,0,d);
b=true;break}}if(!b){e.push(d)}},addEventListener:function(b,d,c,a){if("addEventListener" in b){b.addEventListener(d,c,a)
}else{b.attachEvent("on"+d,c,a)}},removeEventListener:function(b,d,c,a){if("removeEventListener" in b){b.removeEventListener(d,c,a)
}else{b.detachEvent("on"+d,c,a)}},stopEventDefault:function(a){a=a||window.event;if("preventDefault" in a){a.preventDefault()
}a.returnValue=false}}))();if(collie.util.getDeviceInfo().ios){window.addEventListener("load",function(){setTimeout(function(){document.body.scrollTop=0
},300)})}if(!Function.prototype.bind){Function.prototype.bind=function(a){if(typeof this!=="function"){throw new TypeError("Function.prototype.bind - what is trying to be bound is not callable")
...
};b.prototype=this.prototype;c.prototype=new b();return c}}})();collie.Effect=function(e){if(this instanceof arguments.callee){throw new Error("You can't create a instance of this")
...
}}},optionSetter:function(b,a){this._htOptionSetter[b]=a},fireEvent:function(e,c){if(this._htHandler[e]!==undefined&&this._htHandler[e].length>0){c=c||{};
...
}}},stop:function(){this._bCanceled=true},isStop:function(){return this._bCanceled}});collie.ImageManager=collie.ImageManager||new (collie.Class({RETRY_COUNT:3,RETRY_DELAY:500,USE_PRERENDERING_DOM:false,$init:function(){this._aImages=[];
this._htImageNames={};this._htImageRetryCount={};this._htImageWhileLoading={};this._nCount=0},_addImage:function(e,f){var d=this._aImages.push({element:e,name:f});
var b=this._htImageNames[f];this._htImageNames[f]=d-1;delete this._htImageRetryCount[f];if(b&&b instanceof Array){for(var c=0,a=b.length;
c<a;c++){b[c](e,f)}b=null}this.fireEvent("process",{name:f,url:e.src,count:d,total:this._nCount,ratio:Math.round((d/this._nCount)*1000)/1000});
...
}var a=this._aImages.splice(this._htImageNames[b],1);this._makeHash();a.onload=null;a.onerror=null;a.src=null;a=null},remove:function(a){this.removeImage(a)
},add:function(){if(typeof arguments[0]=="object"){this.addImages.apply(this,arguments)}else{this.addImage.apply(this,arguments)
}},addImages:function(f,h,e){var a=null;var d=null;var j=0;var g=0;var b=[];for(var c in f){j++}if(h&&h!==null){a=(function(){g++;
if(g>=j){h(f)}}).bind(this)}if(e&&e!==null){d=(function(k,m,l){b.push([k,m,l]);if(b.length+g>=j){e(b)}}).bind(this)}for(var c in f){this.addImage(c,f[c],a,d)
}},addImage:function(d,c,e,b){if(this.getImage(d)){if(e&&e!==null){e(this.getImage(d),d,c)}return}if((d in this._htImageWhileLoading)&&this._addMarkCallback(d,e,b)){return
}this._nCount++;this._markImage(d);var a=new Image();if(this.USE_PRERENDERING_DOM&&collie.Renderer.getRenderingMode()=="dom"&&collie.util.getSupportCSS3d()&&!collie.util.getDeviceInfo().android){a.style.webkitTransform="translateZ(0)";
...
this._nCount=0},cancelGetImage:function(c,d){if(this._htImageNames[c] instanceof Array){for(var b=0,a=this._htImageNames[c].length;
b<a;b++){if(this._htImageNames[c][b]==d){this._htImageNames[c].splice(b,1);return}}}}},collie.Component))();collie.Matrix={multiple:function(c,b){var j=[];
for(var k=0,g=b.length;k<g;k++){var a=[];for(var f=0,e=b[0].length;f<e;f++){var l=0;for(var h=0,d=c[0].length;h<d;h++){l+=c[k][h]*b[h][f]
}a.push(l)}j.push(a)}return j},translate:function(b,a){return[[1,0,b],[0,1,a],[0,0,1]]},scale:function(b,a){return[[b,0,0],[0,a,0],[0,0,1]]
},rotate:function(b){var c=collie.util.toRad(b);var a=Math.cos(c);var d=Math.sin(c);return[[a,-d,0],[d,a,0],[0,0,1]]},transform:function(b,e,d){var c=this.multiple(b,[[e],[d],[1]]);
return{x:c[0][0],y:c[1][0]}}};collie.Transform={_htBoundary:{left:0,top:0,right:0,bottom:0},getBoundary:function(d){var c=d.get();
var e=d.getRelatedPosition();var a=[[0,0],[c.width,0],[c.width,c.height],[0,c.height]];var f=this.points(d,a);var b=collie.util.getBoundary(f);
this._htBoundary.left=b.left+e.x,this._htBoundary.right=b.right+e.x,this._htBoundary.bottom=b.bottom+e.y,this._htBoundary.top=b.top+e.y;
return this._htBoundary},points:function(f,c){if(!this._isUseTransform(f)){return c}var b=this.getMatrix(f);var e=[];for(var d=0,a=c.length;
d<a;d++){var g=collie.Matrix.transform(b,c[d][0],c[d][1]);e.push([g.x,g.y])}return e},getMatrix:function(d){var b=d.getOrigin();
var c=d.get();var a=collie.Matrix.translate(b.x,b.y);if(c.angle!==0){a=collie.Matrix.multiple(a,collie.Matrix.rotate(c.angle))
}if(c.scaleX!==1||c.scaleY!==1){a=collie.Matrix.multiple(a,collie.Matrix.scale(c.scaleX,c.scaleY))}a=collie.Matrix.multiple(a,collie.Matrix.translate(-b.x,-b.y));
return a},_isUseTransform:function(b){var a=b.get();return(a.scaleX!==1||a.scaleY!==1||a.angle!==0)}};collie.LayerCanvas=collie.Class({$init:function(a){this._oLayer=a;
this._oEvent=a.getEvent();this._initCanvas()},_initCanvas:function(){var a=this._getLayerSize();this._elCanvas=document.createElement("canvas");
this._elCanvas.width=a.width;this._elCanvas.height=a.height;this._elCanvas.className="_collie_layer";this._elCanvas.style.position="absolute";
this._elCanvas.style.left=this._oLayer.option("x")+"px";this._elCanvas.style.top=this._oLayer.option("y")+"px";if(collie.Renderer.isRetinaDisplay()){this._elCanvas.style.width=(a.width/2)+"px";
this._elCanvas.style.height=(a.height/2)+"px"}this._oContext=this._elCanvas.getContext("2d")},_getLayerSize:function(b,a){b=b||this._oLayer.option("width");
a=a||this._oLayer.option("height");if(collie.Renderer.isRetinaDisplay()){b*=2;a*=2}return{width:b,height:a}},getContext:function(){return this._oContext
},getElement:function(){return this._elCanvas},clear:function(a){a=a||this.getContext();a.clearRect(0,0,this._elCanvas.width,this._elCanvas.height)
},resize:function(c,b,f){var a=this._getLayerSize(c,b);if(f){this._elCanvas.style.width=c+"px";this._elCanvas.style.height=b+"px";
var d=c/this._oLayer.option("width");var h=b/this._oLayer.option("height");this._oEvent.setEventRatio(d,h)}else{var e=typeof c=="number"?a.width:this._elCanvas.width;
var g=typeof b=="number"?a.height:this._elCanvas.height;this.clear(this._oContext);this._oLayer.setChanged();this._elCanvas.width=e;
this._elCanvas.height=g;if(collie.Renderer.isRetinaDisplay()){this._elCanvas.style.width=e/2+"px";this._elCanvas.style.height=g/2+"px"
}}}});collie.LayerDOM=collie.Class({$init:function(a){this._oLayer=a;this._oEvent=a.getEvent();this._htOption=a.option();
this._initElement();this._rxDisplayObjectId=new RegExp(collie.DisplayObjectDOM.ID+"([0-9]+)")},_initElement:function(){this._el=document.createElement("div");
...
}else{this._el.style[collie.util.getCSSPrefix("transform",true)]="scale("+c+", "+e+")"}}else{this._el.style.width=b+"px";
this._el.style.height=a+"px"}}});collie.LayerEvent=collie.Class({THRESHOLD_CLICK:7,$init:function(a){this._oLayer=a;this._bHasTouchEvent=!!("ontouchstart" in window);
this._fOnEvent=this._onEvent.bind(this);this._oMousedownObject=null;this._htEventRatio={width:1,height:1};this._bAttached=false
},attachEvent:function(){var a=collie.Renderer.getElement();if(this._oLayer.option("useEvent")){if(this._bHasTouchEvent){collie.util.addEventListener(a,"touchstart",this._fOnEvent);
...
collie.util.addEventListener(a,"mousemove",this._fOnEvent)}this._bAttached=true}},detachEvent:function(){var a=collie.Renderer.getElement();
...
collie.util.removeEventListener(a,"mousemove",this._fOnEvent)}this._bAttached=false}},_onEvent:function(n){if(!this._oLayer.option("useEvent")){return
...
var o=this._bHasTouchEvent?p.pageY:p.pageY||p.clientY+h.scrollTop-m.body.clientTop;var b=n.type;var l=null;var f=collie.Renderer.getPosition();
var k=q-f.x-this._oLayer.option("x");var j=o-f.y-this._oLayer.option("y");if(b=="touchcancel"){if(this._htEventStartPos!==null){k=this._htEventStartPos.x;
j=this._htEventStartPos.y}}b=this._convertEventType(b);if(b=="mousemove"||b=="mousedown"){if(collie.Renderer.isPreventDefault()){collie.util.stopEventDefault(n)
}}if(b=="mousedown"){this._htEventStartPos={x:k,y:j}}var a=this._fireEvent(n,b,k,j);if(b=="mouseup"){var d=this.THRESHOLD_CLICK*this._htEventRatio.width;
var c=this.THRESHOLD_CLICK*this._htEventRatio.height;if(this._htEventStartPos&&this._htEventStartPos.x-d<=k&&k<=this._htEventStartPos.x+d&&this._htEventStartPos.y-c<=j&&j<=this._htEventStartPos.y+c){this._fireEvent(n,"click",k,j)
...
}a/=this._htEventRatio.width;f/=this._htEventRatio.height;var b=d.getBoundary();if(b.left<=a&&a<=b.right&&b.top<=f&&f<=b.bottom){if(!d.get("hitArea")){return true
}else{var e=d.getRelatedPosition();a-=e.x;f-=e.y;var c=d.get("hitArea");c=collie.Transform.points(d,c);return this._isPointInPolygon(c,a,f)
}}return false},_isPointInPolygon:function(c,g,f){var e=false;for(var d=0,b=c.length-1,a=c.length;d<a;b=d++){if((c[d][1]>f)!=(c[b][1]>f)&&(g<(c[b][0]-c[d][0])*(f-c[d][1])/(c[b][1]-c[d][1])+c[d][0])){e=!e
...
this._htEventRatio.height=a||this._htEventRatio.height},getEventRatio:function(){return this._htEventRatio}});collie.Layer=collie.Class({type:"layer",$init:function(a){this.option({x:0,y:0,width:320,height:480,useEvent:true,visible:true,freeze:false});
this._sAlignLeft=null;this._sAlignTop=null;if(a.x=="left"||a.x=="right"||a.x=="center"){this._sAlignLeft=a.x;a.x=0}if(a.y=="top"||a.y=="bottom"||a.y=="center"){this._sAlignTop=a.y;
a.y=0}if(a!==undefined){this.option(a)}this.drawCount=0;this.optionSetter("visible",this._setVisible.bind(this));this._elParent=null;
this._bChanged=false;this._aDisplayObjects=[];this._bLoaded=false;this._oEvent=new collie.LayerEvent(this);this._makeDrawing();
this._setVisible()},_makeDrawing:function(){this._oDrawing=collie.Renderer.getRenderingMode()=="dom"?new collie.LayerDOM(this):new collie.LayerCanvas(this)
},getDrawing:function(){return this._oDrawing},getEvent:function(){return this._oEvent},load:function(b,a){this.unload();
this._bLoaded=true;this._elParent=b;this._elParent.style.width=Math.max(parseInt(this._elParent.style.width||0,10),this.option("width"))+"px";
this._elParent.style.height=Math.max(parseInt(this._elParent.style.height||0,10),this.option("height"))+"px";this.getElement().style.zIndex=a;
if(this._sAlignLeft!==null){this.offset(this._sAlignLeft,null,true)}if(this._sAlignTop!==null){this.offset(null,this._sAlignTop,true)
...
},resize:function(b,a,c){if(!c){this.option("width",b||this.option("width"));this.option("height",a||this.option("height"))
...
this._elParent.style.height=Math.max(parseInt(this._elParent.style.height||0,10),a||this.option("height"))+"px"}},offset:function(d,c,b){var a=this.getElement();
if(typeof d!=="undefined"&&d!==null){switch(d){case"left":d=0;break;case"right":d=parseInt(this._elParent.style.width,10)-this._htOption.width;
...
if(!b){this._sAlignTop=null}}}},collie.Component);collie.DisplayObjectCanvas=collie.Class({$init:function(a){this._oDisplayObject=a;
...
this._bIsRetinaDisplay=collie.Renderer.isRetinaDisplay()},unload:function(){this._oLayer=null;this._oContext=null},draw:function(B,c,b,t,H,w){var G=w?true:false;
w=w||this._oContext;var h=this._bUseCache?this._oContextCache:w;var I=w;var z=this._htInfo;var l=this._oDisplayObject.getDirty();
var D=this._oDisplayObject.getOrigin();var y=z.width;var L=z.height;var e=D.x;var d=D.y;var C=c;var A=b;var k=(this._bIsRetinaDisplay?2:1);
var a=c*k;var u=b*k;var p=0;var x=false;var q=w;if(z.useCache){w=this._oContextCache}if(this._bIsRetinaDisplay){c*=2;b*=2;
e*=2;d*=2;y*=2;L*=2}if(this._bUseCache||z.scaleX!=1||z.scaleY!=1||z.angle!==0||z.opacity!=1){x=true;if(this._bUseCache){q=!G?this._oContext:I
}q.save();q.translate(c+e,b+d);if(z.opacity!=1){p=q.globalAlpha;q.globalAlpha=q.globalAlpha===0?z.opacity:q.globalAlpha*z.opacity
}if(z.angle!==0){q.rotate(collie.util.toRad(z.angle))}if(z.scaleX!==1||z.scaleY!==1){q.scale(z.scaleX,z.scaleY)}q.translate(-e,-d);
...
if(z.backgroundColor){h.fillStyle=z.backgroundColor;h.fillRect(c,b,y,L)}if("onCanvasDraw" in this._oDisplayObject){this._oDisplayObject.onCanvasDraw(this._htEvent)
}if(this._oDisplayObject.getImage()){var j=this._oDisplayObject.getImage();var K=this._oDisplayObject.getImageSize();if(z.backgroundRepeat&&z.backgroundRepeat!="no-repeat"){var n=(z.backgroundRepeat=="repeat"||z.backgroundRepeat=="repeat-x")?Math.ceil(z.width/K.width):1;
var F=(z.backgroundRepeat=="repeat"||z.backgroundRepeat=="repeat-y")?Math.ceil(z.height/K.height):1;if(n>0||F>0){for(var o=0;
o<n;o++){for(var E=0;E<F;E++){var g=o*K.width+K.width;var f=E*K.height+K.height;var s=g>z.width?K.width-(g-z.width):K.width;
var v=f>z.height?K.height-(f-z.height):K.height;this.drawImage(h,0,0,s,v,(c/k)+o*K.width,(b/k)+E*K.height,s,v)}}}}else{this.drawImage(h,z.offsetX,z.offsetY,Math.min(K.width,z.width),Math.min(K.height,z.height),c/k,b/k,z.width,z.height)
}}}if(z.debugHitArea&&z.hitArea){if(this._oDebugHitArea===null){this._oDebugHitArea=new collie.Polyline({x:0,y:0,width:z.width,height:z.height,strokeColor:z.debugHitArea===true?"yellow":z.debugHitArea,strokeWidth:3}).addTo(this._oDisplayObject);
this._oDebugHitArea.setPointData(z.hitArea)}}if(this._oDisplayObject.hasChild()&&(!z.useCache||(this._oDisplayObject.isChanged()&&!this._oDisplayObject.isChanged(true)))){var m=this._oDisplayObject.getChildren();
for(var J=0,r=m.length;J<r;J++){m[J].update(B,z.useCache||x?0:C,z.useCache||x?0:A,t,H,G||z.useCache?w:null);m[J].unsetChanged();
m[J]._resetDirty()}}if(z.useCache){(G?I:this._oContext).drawImage(w.canvas,0,0)}this._oLayer.drawCount++;if(x){q.restore()
...
this._bIsIEUnder8=this._htDeviceInfo.ie&&this._htDeviceInfo.ie<9;this._bUseTranslateZ=true;this._bIsRetinaDisplay=null;this._htEvent={};
this._oEmptyObject={};this._sCacheTransformValue=null;this._initElement()},_initElement:function(){this._elContainer=document.createElement("div");
...
}else{this._elContainerStyle.opacity=e.opacity}}if(h.zIndex){this._elContainerStyle.zIndex=e.zIndex}if(h.backgroundColor){this._elStyle.backgroundColor=e.backgroundColor
...
},_drawImage:function(e,c){var a=this._oDisplayObject.getImage();var d=e.backgroundRepeat&&e.backgroundRepeat!="no-repeat";
var f=this._oDisplayObject.getImageSize();if(c.backgroundImage||c.backgroundRepeat){if(this._elImage!==null&&(!e.backgroundImage||(e.backgroundRepeat&&e.backgroundRepeat!="no-repeat"))){this._el.removeChild(this._elImage);
this._elImage=null}if(e.backgroundImage&&a){if(!d&&e.backgroundImage){this._elImage=a.cloneNode();var b=this._elImage.style;
if(this._bSupportCSS3d&&this._bUseTranslateZ){b[this._sTransform]="translateZ(0)"}b.position="absolute";b.top=0;b.left=0;
b.width=f.width+"px";b.height=f.height+"px";this._el.appendChild(this._elImage)}else{if(d){this._elStyle.backgroundImage='url("'+a.src+'")';
this._elStyle.backgroundRepeat=e.backgroundRepeat}}}}if(e.backgroundImage&&this._elImage!==null){if(this._bIsRetinaDisplay&&d&&(c.width||htDirth.height||c.backgroundRepeat||c.backgroundImage)){this._elStyle.backgroundSize=e.width+"px "+e.height+"px"
}if(c.offsetX||c.offsetY){if(this._bUseTransform){this._elImage.style[this._sTransform]=this._makeTranslate(-e.offsetX,-e.offsetY,0)
}else{this._elImage.style.left=-e.offsetX+"px";this._elImage.style.top=-e.offsetY+"px"}}}},_makeTranslate:function(e,d,c){var a="";
...
this._htOption={name:"",width:"auto",height:"auto",x:0,y:0,zIndex:0,opacity:1,angle:0,scaleX:1,scaleY:1,originX:"center",originY:"center",offsetX:0,offsetY:0,spriteX:null,spriteY:null,spriteLength:0,rangeX:null,rangeY:null,positionRepeat:false,backgroundColor:"",backgroundImage:"",backgroundRepeat:"no-repeat",hitArea:null,debugHitArea:false,useCache:false,useEvent:"auto",visible:true};
...
this._bCustomSize=false;this._aChangedQueue=null;this._htGetImageData=null;this._htRelatedPosition={};this._sRenderingMode=collie.Renderer.getRenderingMode();
this._bRetinaDisplay=collie.Renderer.isRetinaDisplay();this._oTimerMove=null;this._nPositionRight=null;this._nPositionBottom=null;
this._nImageWidth=0;this._nImageHeight=0;this._htImageSize=null;this._htOrigin={x:0,y:0};this.set(this._htOption);this._bIsSetOption=true
},set:function(e,d,b,a){if(typeof e=="object"){for(var c in e){this.set(c,e[c])}}else{if(this._bIsSetOption&&this._htOption[e]===d){return
}if(e=="width"||e=="height"){if(d!="auto"){this._bCustomSize=true}else{if(d=="auto"&&this.getImage()!==null){d=this.getImageSize()[e]
}else{d=100}}}this._htOption[e]=d;this.setDirty(e);if(!b){this._setter(e,d)}if(!a){this.setChanged((e=="x"||e=="y"||e=="angle"||e=="scaleX"||e=="scaleY"||e=="opacity")?true:false)
...
}}}if(b=="x"||b=="y"){if(typeof a=="string"){this.align(b=="x"?a:false,b=="y"?a:false)}this._fixPosition()}if(b=="backgroundImage"){this.setImage(a)
}if(b=="spriteX"||b=="spriteY"){this._setSpritePosition(b,a)}if(b=="hitArea"&&a instanceof Array){this._makeHitAreaBoundary()
}if(b=="width"||b=="height"||b=="originX"||b=="originY"){this._setOrigin()}if((b=="backgroundRepeat"&&a!="no-repeat")){this.set("useCache",true)
}if(b=="useCache"&&this._oDrawing!==null&&this._oDrawing.loadCache){if(a){this._oDrawing.loadCache()}else{this._oDrawing.unloadCache()
}}},get:function(a){if(!a){return this._htOption}else{return this._htOption[a]}},setDirty:function(b){if(this._htDirty===null){this._htDirty={}
}if(b===undefined){for(var a in this._htOption){this._htDirty[a]=true}}else{this._htDirty[b]=true}},getDirty:function(a){if(!a){return this._htDirty
...
if(!this._bCustomSize){this.set({width:this._htImageSize.width,height:this._htImageSize.height})}this._setSpritePosition("spriteX",this.get("spriteX"));
this._setSpritePosition("spriteY",this.get("spriteY"));this.setDirty("backgroundImage");this.setChanged()},getDrawing:function(){return this._oDrawing
...
}collie.DisplayObject.htFactory[this._sId]=this;var d=this.get();this._oLayer=b;this._makeDrawing();this._oDrawing.load();
this.setChanged();if(typeof d.x=="string"||typeof d.y=="string"){this.align(typeof d.x=="string"?d.x:false,typeof d.y=="string"?d.y:false)
...
b<a;b++){this._aDisplayObjects[b].unsetLayer()}this._oDrawing.unload();this.setDirty();this.setChanged();this._oDrawing=null;
this._oLayer=null;delete collie.DisplayObject.htFactory[this._sId]}},_makeDrawing:function(){if(this._oDrawing===null){this._oDrawing=this._sRenderingMode=="dom"?new collie.DisplayObjectDOM(this):new collie.DisplayObjectCanvas(this)
...
}},update:function(b,j,g,a,e,h){if(this._sRenderingMode=="canvas"&&!this.get("visible")){this.unsetChanged();return}var c=this.get();
j+=c.x;g+=c.y;if((this._sRenderingMode=="dom"&&this.isChanged())||(this._sRenderingMode=="canvas"&&(j+c.width>=0||j<=a||g+c.height>=0||g<=e))){this._oDrawing.draw(b,j,g,a,e,h)
}this.unsetChanged();this._resetDirty();if(this._sRenderingMode=="canvas"||!this.get("visible")){return}if(this.hasChild()){for(var d=0,f=this._aDisplayObjects.length;
d<f;d++){this._aDisplayObjects[d].update(b,j,g,a,e)}}},getRelatedPosition:function(){this._htRelatedPosition.x=this.get("x");
this._htRelatedPosition.y=this.get("y");var a=this;while(a._oParent){a=a._oParent;this._htRelatedPosition.x+=a.get("x");this._htRelatedPosition.y+=a.get("y")
}return this._htRelatedPosition},getBoundary:function(){return collie.Transform.getBoundary(this)},getHitAreaBoundary:function(){if(!this.get("hitArea")){return this.getBoundary()
}else{if(this.get("hitArea") instanceof Array){var c=this.getRelatedPosition();var a=collie.Transform.points(this,collie.util.getBoundaryToPoints(this._htHitAreaBoundary));
var b=collie.util.getBoundary(a);return{left:b.left+c.x,right:b.right+c.x,top:b.top+c.y,bottom:b.bottom+c.y}}else{return this.get("hitArea").getBoundary()
}}},getOrigin:function(){return this._htOrigin},_setOrigin:function(){var a=this.get();switch(a.originX){case"left":this._htOrigin.x=0;
break;case"right":this._htOrigin.x=a.width;break;case"center":this._htOrigin.x=a.width/2;break;default:this._htOrigin.x=parseInt(a.originX,10)
}switch(a.originY){case"top":this._htOrigin.y=0;break;case"bottom":this._htOrigin.y=a.height;break;case"center":this._htOrigin.y=a.height/2;
break;default:this._htOrigin.y=parseInt(a.originY,10)}},_fixPosition:function(){var c=this.get();var g=c.x;var f=c.y;var e;
var b;var d;var a;if(c.rangeX){e=c.rangeX[0];b=c.rangeX[1];if(c.positionRepeat){if(g<e){do{g+=(b-e)}while(g<e)}else{if(g>b){do{g-=(b-e)
}while(g>b)}}}else{g=Math.max(e,g);g=Math.min(b,g)}if(g!=c.x){this.set("x",g,true)}}if(c.rangeY){d=c.rangeY[0];a=c.rangeY[1];
if(c.positionRepeat){if(f<d){do{f+=(a-d)}while(f<d)}else{if(f>a){do{f-=(a-d)}while(f>a)}}}else{f=Math.max(d,f);f=Math.min(a,f)
}if(f!=c.y){this.set("y",f,true)}}},_makeHitAreaBoundary:function(){this._htHitAreaBoundary=collie.util.getBoundary(this.get("hitArea"))
},align:function(f,b,e){if(!this.getLayer()){return}e=e||this.getParent();var d=this.get();var c=0;var a=0;var h=0;var g=0;
if(e){c=e.get("width");a=e.get("height")}else{c=this.getLayer().option("width");a=this.getLayer().option("height")}if(f!==false){h=(f=="right")?c-d.width:c/2-d.width/2;
this.set("x",h)}if(b!==false){g=(b=="bottom")?a-d.height:a/2-d.height/2;this.set("y",g)}},right:function(b){var a=0;if(this._oParent){a=this._oParent.get("width")
}if(!a&&this._oLayer){a=this._oLayer.option("width")}if(a){this.set("x",a-(this._htOption.width+b))}else{this._nPositionRight=b
}return this},bottom:function(b){var a=0;if(this._oParent){a=this._oParent.get("height")}if(!a&&this._oLayer){a=this._oLayer.option("height")
...
}},_setSpritePosition:function(n,f){if(this._elImage&&f!==null){var a=this.getImageSize();var g=this.get();var l=g.width;
var m=g.height;var h=g.spriteLength-1;var k=(a.width/g.width)-1;var j=(a.height/g.height)-1;var c=a.width-1;var b=a.height-1;
if(h>=0&&m<a.height){c=k*a.width;b=j*a.height}switch(n){case"spriteX":var e=0;var d=0;if(h>k&&m<a.height){d=Math.floor(f/(k+1))*m;
e=(f%(k+1))*l}else{e=Math.min(f,k)*l}this.set("offsetX",e,true);this.set("offsetY",d,true);break;case"spriteY":f=Math.min(f,j);
this.set("offsetY",f*m,true);break}}},hasAttachedHandler:function(){if(this._htHandler&&(("click" in this._htHandler)&&this._htHandler.click.length>0)||(("mousedown" in this._htHandler)&&this._htHandler.mousedown.length>0)||(("mouseup" in this._htHandler)&&this._htHandler.mouseup.length>0)){return true
...
},toString:function(){return"DisplayObject"+(this.get("name")?" "+this.get("name"):"")+" #"+this.getId()+(this.getImage()?"(image:"+this.getImage().src+")":"")
}},collie.Component);collie.DisplayObject._idx=0;collie.DisplayObject.htFactory={};collie.MovableObject=collie.Class({$init:function(a){this.option({velocityX:0,velocityY:0,velocityRotate:0,forceX:0,forceY:0,forceRotate:0,mass:1,friction:0,useRealTime:true},null,true)
},update:function(d,f,e,a,c){var b=Math.max(17,d)/1000;if(!this.get("useRealTime")){b=1}this._applyForce(b);this._applyRotation(b);
this.constructor.$super.update.call(this,d,f,e,a,c);if(this.get("velocityX")!==0||this.get("velocityY")!==0||this.get("velocityRotate")!==0||this.get("forceX")!==0||this.get("forceY")!==0||this.get("forceRotate")!==0){this.setChanged(true)
}},_getValueDirection:function(a){return Math.abs(a)/a},_applyRotation:function(b){if(this._htOption.forceRotate!==0){this.set("velocityRotate",this._htOption.velocityRotate+this._htOption.forceRotate)
}if(this._htOption.velocityRotate!==0){var a=collie.util.fixAngle(collie.util.toRad(this._htOption.angle+this._htOption.velocityRotate*b));
this.set("angle",Math.round(collie.util.toDeg(a)*1000)/1000)}},_applyForce:function(f){var e=this.get();var d=e.velocityX;
var c=e.velocityY;var h=e.x;var g=e.y;d+=(e.forceX/e.mass)*f;c+=(e.forceY/e.mass)*f;var b=e.friction*d*e.mass*f;var a=e.friction*c*e.mass*f;
if(d!==0){d=(this._getValueDirection(d)!==this._getValueDirection(d-b))?0:d-b}if(c!==0){c=(this._getValueDirection(c)!==this._getValueDirection(c-a))?0:c-a
}h+=d*f;g+=c*f;d=Math.floor(d*1000)/1000;c=Math.floor(c*1000)/1000;if(e.friction&&Math.abs(d)<0.05){d=0}if(e.friction&&Math.abs(c)<0.05){c=0
}if(h!=e.x||g!=e.y||d!=e.velocityX||c!=e.velocityY){this.set({x:h,y:g,velocityX:d,velocityY:c})}},toString:function(){return"MovableObject"+(this._htOption.name?" "+this._htOption.name:"")+" #"+this.getId()+(this.getImage()?"(image:"+this.getImage().src+")":"")
}},collie.DisplayObject);collie.Rectangle=collie.Class({$init:function(a){this.option({radius:0,strokeColor:"",strokeWidth:0,fillColor:""},null,true);
this._sBorderRadius=collie.util.getCSSPrefix("border-radius",true)},onDOMDraw:function(a){if(this._bChanged){var b=this.get();
if(this.get("radius")){a.element.style[this._sBorderRadius]=this.get("radius")+"px";a.element.style.borderRadius=this.get("radius")+"px"
}if(this.get("fillColor")){a.element.style.backgroundColor=this.get("fillColor")}if(this.get("strokeWidth")){a.element.style.border=this.get("strokeWidth")+"px solid "+this.get("strokeColor")
}this._bChanged=false}},onCanvasDraw:function(d){var g=this.get();var f=d.context;var b=g.radius;var e=collie.Renderer.isRetinaDisplay();
var c=g.width;var a=g.height;var h=g.strokeWidth;if(e){c*=2;a*=2;b*=2;h*=2}if(this.get("fillColor")&&!g.backgroundGradient){f.fillStyle=this.get("fillColor")
}if(this.get("strokeColor")){f.strokeStyle=this.get("strokeColor")}if(this.get("strokeWidth")){f.lineWidth=h}if(b){f.save();
f.translate(d.x,d.y);f.beginPath();f.moveTo(b,0);f.lineTo(c-b,0);f.quadraticCurveTo(c,0,c,b);f.lineTo(c,a-b);f.quadraticCurveTo(c,a,c-b,a);
f.lineTo(b,a);f.quadraticCurveTo(0,a,0,a-b);f.lineTo(0,b);f.quadraticCurveTo(0,0,b,0);f.closePath();f.restore();if(this.get("fillColor")){f.fill()
}if(this.get("strokeWidth")){f.stroke()}}else{if(this.get("fillColor")){f.fillRect(d.x,d.y,c,a)}if(this.get("strokeWidth")){f.strokeRect(d.x,d.y,c,a)
...
}},collie.DisplayObject);collie.Circle=collie.Class({$init:function(a){this.option({radius:0,strokeColor:"",strokeWidth:0,fillColor:"",startAngle:0,endAngle:360,anticlockwise:false},null,true)
},onCanvasDraw:function(k){var a=this.get();var h=k.context;var j=k.x;var g=k.y;var c=collie.Renderer.isRetinaDisplay();var b=a.radius;
var e=a.strokeWidth;var d=a.width;var f=a.height;if(c){d*=2;f*=2;b*=2;e*=2}if(a.fillColor&&!a.backgroundGradient){h.fillStyle=a.fillColor
}if(a.strokeColor){h.strokeStyle=a.strokeColor}if(e){h.lineWidth=e}if(b){h.beginPath();h.moveTo(j+d/2,g+f/2);h.arc(j+d/2,g+f/2,b,collie.util.toRad(a.startAngle),collie.util.toRad(a.endAngle),a.anticlockwise);
h.closePath();if(this.get("fillColor")){h.fill()}if(this.get("strokeWidth")){h.stroke()}}},toString:function(){return"Circle"+(this._htOption.name?" "+this._htOption.name:"")+" #"+this.getId()+(this.getImage()?"(image:"+this.getImage().src+")":"")
}},collie.DisplayObject);collie.Polyline=collie.Class({$init:function(a){this.option({strokeColor:"",strokeWidth:0,fillColor:"",usePolygon:false},null,true);
this._aPointData=[];this._htPointBoundary={left:null,right:null,top:null,bottom:null}},setPointData:function(a,b){this._aPointData=a;
...
this.setChanged();if(!a){this._expandBoundary(c,b)}},resetPointData:function(){this._aPointData=[];this._htPointBoundary={left:null,right:null,top:null,bottom:null};
this.setChanged()},_expandBoundary:function(f,e,b){if(f instanceof Array){for(var c=0,a=f.length;c<a;c++){this._expandBoundary(f[c][0],f[c][1],true)
}}else{this._htPointBoundary.left=this._htPointBoundary.left===null?f:Math.min(f,this._htPointBoundary.left);this._htPointBoundary.right=this._htPointBoundary.right===null?f:Math.max(f,this._htPointBoundary.right);
this._htPointBoundary.top=this._htPointBoundary.top===null?e:Math.min(e,this._htPointBoundary.top);this._htPointBoundary.bottom=this._htPointBoundary.bottom===null?e:Math.max(e,this._htPointBoundary.bottom)
}if(!b){var d=this.get("strokeWidth")*(collie.Renderer.isRetinaDisplay()?2:1);this.set({width:(this._htPointBoundary.right-this._htPointBoundary.left)+d*2,height:this._htPointBoundary.bottom-this._htPointBoundary.top+d*2})
}},onCanvasDraw:function(c){if(this._aPointData.length<2){return}var g=this.get();var f=c.context;var d=collie.Renderer.isRetinaDisplay();
var h=g.strokeWidth;var b=(d?2:1);f.save();f.translate(c.x,c.y);if(d){h*=2}if(g.fillColor&&!g.backgroundGradient){f.fillStyle=g.fillColor
}if(g.strokeColor){f.strokeStyle=g.strokeColor}if(h){f.lineWidth=h}f.beginPath();f.moveTo(this._aPointData[0][0]*b,this._aPointData[0][1]*b);
for(var e=1,a=this._aPointData.length;e<a;e++){f.lineTo(this._aPointData[e][0]*b,this._aPointData[e][1]*b)}if(g.usePolygon&&this._aPointData[0][0]!=this._aPointData[this._aPointData.length-1][0]||this._aPointData[0][1]!=this._aPointData[this._aPointData.length-1][1]){f.lineTo(this._aPointData[0][0]*b,this._aPointData[0][1]*b)
}f.closePath();if(this.get("fillColor")){f.fill()}if(this.get("strokeWidth")){f.stroke()}f.restore()},toString:function(){return"Polyline"+(this.get("name")?" "+this.get("name"):"")+" #"+this.getId()+(this.getImage()?"(image:"+this.getImage().src+")":"")
...
this._elText=null;this._nTextWidth=0;this._aCallbackTextWidth=[];this._nRatio=collie.Renderer.getRenderingMode()=="canvas"&&collie.Renderer.isRetinaDisplay()?2:1
},_initElement:function(){if(this._elText===null){this._elText=document.createElement("div");this._elText.style.display="inline";
this.getDrawing().getItemElement().appendChild(this._elText)}},onCanvasDraw:function(a){this._oContext=a.context;var b=this._getMaxWidth();
this._oContext.font=this._getFontText();this._oContext.fillStyle=this._htOption.fontColor;this._oContext.textBaseline="top";
this._fillTextMultiline(this._wordWrap(b).split("\n"),a.x,a.y);this._triggerGetTextWidth()},onDOMDraw:function(a){this._initElement();
var c=this.getDrawing();var e=a.element;var b=this._sText.replace(/\n/g,"<br />");var d=e.style;d.font=this._getFontText();
d.color=this._htOption.fontColor;d.padding=this._getPadding().replace(/ /g,"px ")+"px";d.width=this._getMaxWidth()+"px";d.height=this._getMaxHeight()+"px";
d.lineHeight=this._getLineHeight()+"px";d.textAlign=this._htOption.textAlign;if(this._elText.innerHTML!=b){this._elText.innerHTML=b
}this.unsetChanged();this._getDOMTextWidth();this._triggerGetTextWidth()},_getDOMTextWidth:function(){if(this._elText!==null){this._nTextWidth=this._elText.offsetWidth
}},_getFontText:function(){return this._htOption.fontWeight+" "+(this._htOption.fontSize*this._nRatio)+"px "+this._htOption.fontFamily
...
this._nTextWidth=Math.max(this._nTextWidth,a)}},_getMaxWidth:function(){return(this.get("width")*this._nRatio)-(this._getPadding("left")+this._getPadding("right"))
},_getMaxHeight:function(){return(this.get("height")*this._nRatio)-(this._getPadding("top")+this._getPadding("bottom"))},_getTopPosition:function(a){return this._getLineHeight()*(a-1)+this._getPadding("top")
},_getPadding:function(b){var e=this.get("padding")||"0 0 0 0";var c=e.split(" ");for(var d=0,a=c.length;d<a;d++){c[d]=parseInt(c[d],10)*this._nRatio
}switch(b){case"top":return c[0];case"right":return c[1];case"bottom":return c[2];case"left":return c[3];default:return c.join(" ")
}},_insertEllipsisText:function(b){var a=this._getMaxWidth();var d="";for(var c=b.length;c>0;c--){d=b.substr(0,c)+this.get("ellipsisString");
if(this._oContext.measureText(d).width<=a){return d}}return b},_wordWrap:function(a,d){var c=d||this._sText;var b=1;if(!c){return""
}d=c.substr(0,1);while(this._oContext.measureText(d).width<=a){b++;if(b>c.length){return d}d=c.substr(0,b);if(c.substr(b-1,1)=="\n"){break
}}b=Math.max(1,b-1);d=c.substr(0,b);if(c.substr(b,1)=="\n"){b++}if(c.length>b){d+="\n"+(this._wordWrap(a,c.substr(b)))}return d
},text:function(a){this._nTextWidth=0;this._aCallbackTextWidth=[];this._sText=a.toString();this.setChanged();return this},getTextWidth:function(a){if(a){this._aCallbackTextWidth.push(a)
}if(this._nTextWidth){this._triggerGetTextWidth();return this._nTextWidth/this._nRatio}},_triggerGetTextWidth:function(){if(this._aCallbackTextWidth.length>0){for(var b=0,a=this._aCallbackTextWidth.length;
b<a;b++){this._aCallbackTextWidth[b](this._nTextWidth/this._nRatio)}this._aCallbackTextWidth=[]}},toString:function(){return"Text"+(this.get("name")?" "+this.get("name"):"")+" #"+this.getId()+(this.getImage()?"(image:"+this.getImage().src+")":"")
...
b<a;b++){this._fCallback[b].set.apply(this._fCallback[b],aArguments)}}else{this._fCallback.set(d)}}else{if(this._fCallback){this._fCallback(e)
}}},setDuration:function(a){this._nDuration=parseInt(a,10)},getDuration:function(){return this._nDuration},setTimerList:function(a){this._oTimerList=a;
if(this._htOption.useAutoStart){this.start()}},getId:function(){return this._nId},run:function(b,a){throw new Error("abstract method")
},reset:function(){throw new Error("abstract method")},stop:function(a){if(this.isPlaying()){if(this._oTimerList!==null){this._oTimerList.remove(this)
}this._bIsPlaying=false;this.reset();if(!a){this.fireEvent("stop")}}},pause:function(){if(this.isPlaying()){this._bIsPlaying=false;
this.fireEvent("pause");if(this._oTimerList!==null){this._oTimerList.remove(this)}}},start:function(){if(!this.isPlaying()){this._bIsPlaying=true;
if(this._oTimerList!==null){this._oTimerList.add(this)}this.fireEvent("start")}},isPlaying:function(){return this._bIsPlaying
},complete:function(){if(this.isPlaying()){if(this._fCallbackComplete){this._fCallbackComplete()}this.fireEvent("complete");
this.stop(true)}}},collie.Component);collie.Animation._idx=0;collie.AnimationTransition=collie.Class({$init:function(c,d,b){this.option({from:null,to:null,set:"",loop:1,effect:collie.Effect.linear});
...
}if(this._nFrameAtRunLastest===null){this.reset()}this.constructor.$super.start.call(this)},move:function(a,d,b,c){},_setDefaultFromValues:function(){var b=null;
...
this.option(a||{});this._nFrameAtRunLastest=null;this._nRunLastestTime=null;this._nRunningTime=null;this.setDuration(d);this.reset()
},reset:function(){this._nCount=0;this._nCountCycle=0;this._nFrameAtRunLastest=null;this._nRunningTime=null;this._nRunLastestTime=null;
this._nValue=(this._htOption.start!==null?this._htOption.start:this._htOption.from)-this._htOption.step},_setterValueSet:function(){var a=this._htOption.valueSet;
if(a&&a instanceof Array){this.option({from:0,to:a.length-1,step:1})}},_setterFPS:function(){if(this._nFPS!==null&&typeof this._htOption.to!="undefined"&&typeof this._htOption.from!="undefined"){var a=(this._htOption.to-this._htOption.from)+1;
...
},run:function(h,e){if(typeof h=="undefined"){h=collie.Renderer.getInfo().frame}if(this._nFrameAtRunLastest>h){this.reset();
return}if(this._nFrameAtRunLastest===null){this._nFrameAtRunLastest=h;this._nRunLastestTime=0;this._nRunningTime=0;e=0}if(!e){e=0
...
var g=(!d.useRealTime)?0:Math.max(1,Math.floor((this._nRunningTime-this._nRunLastestTime)/this._nTerm))-1;var f=this._nCountCycle;
if(this._nRunningTime===0||this._nRunLastestTime+this._nTerm<=this._nRunningTime){if(this._nValue===d.to){this._nValue=d.from-d.step
}this._nValue+=(d.step*(1+g));this._nCount+=(1+g);if(d.from<=d.to?this._nValue>=d.to:this._nValue<=d.to){var b=(this._nValue-d.to)/d.step;
var a=Math.ceil(b/(this._nTotalCount+1));b=b%(this._nTotalCount+1);if(b){this._nCountCycle+=a;this._nValue=d.from+(b-1)*d.step
}else{this._nCountCycle+=1;this._nValue=d.to}}this._htCallback.timer=this;this._htCallback.frame=h;this._htCallback.duration=this._nDuration;
this._htCallback.count=this._nCount;this._htCallback.skippedCount=g;this._htCallback.runningTime=this._nRunningTime;this._htCallback.value=this.getValue();
...
this.triggerCallback(this._htCallback);if(f!==this._nCountCycle){this.fireEvent("end")}if(d.loop&&this._nCountCycle>=d.loop){this.complete();
return}this._nFrameAtRunLastest=h;this._nRunLastestTime=this._nRunningTime}}},collie.Animation);collie.AnimationDelay=collie.Class({$init:function(a,b){this.reset()
},reset:function(){this._nFrameAtRunLastest=null;this._nRunningTime=null;this._nRunLastestTime=null},run:function(b,a){if(b===undefined){b=collie.Renderer.getInfo().frame
}if(this._nFrameAtRunLastest>b){this.reset();return}if(this._nFrameAtRunLastest===null){this._nFrameAtRunLastest=b;this._nRunLastestTime=0;
this._nRunningTime=0;a=0}this._nRunningTime+=a;if(this._nRunLastestTime+this._nDuration<=this._nRunningTime){if(this._fCallback){this._fCallback({timer:this,frame:b,duration:this._nDuration,runningTime:this._nRunningTime})
...
case"cycle":e=new collie.AnimationCycle(d,f,b);break;default:throw new Error(c+" timer is not defined")}this._addTimeline(a,e);
return e},_addTimeline:function(a,b){this._htAnimations[a]=this._htAnimations[a]||[];this._htAnimations[a].push(b);if(this._aTimeline!==null){this.reset()
}},remove:function(b,c){if(this._htAnimations&&this._htAnimations[b]){for(var a=0;a<this._htAnimations[b].length;a++){if(typeof c=="undefined"||c==this._htAnimations[b][a]){this._htAnimations[b][a].stop();
...
for(var a in this._htAnimations){this._aTimeline.push(a)}this._aTimeline.sort()},getAnimation:function(a){return(this._htAnimations&&this._htAnimations[a])?this._htAnimations[a]:false
},getRunningTime:function(){return this._nRunningTime||0},getCycle:function(){return this._nCountCycle||0},reset:function(){this._nFrameAtRunLastest=null;
this._nRunningTime=null;this._aTimeline=null;this._aRunningAnimation=null;this._nCountCycle=0;this._initTimeline()},run:function(e,d){if(e===undefined){e=collie.Renderer.getInfo().frame
...
},run:function(b,a){if(this._aAnimations.length<1){return}if(b===undefined){b=collie.Renderer.getInfo().frame}if(this._nFrameAtRunLastest>b){this.reset();
return}if(this._nFrameAtRunLastest===null){this._nFrameAtRunLastest=b}if(this._nAnimationIdx===null){this.next()}this._aAnimations[this._nAnimationIdx].run(b,a)
}},collie.Animation);collie.TimerList=collie.Class({$init:function(){this._aList=[]},add:function(a){this._aList.unshift(a)
},remove:function(c){for(var b=0,a=this._aList.length;b<a;b++){if(this._aList[b]==c){this._aList.splice(b,1);break}}},removeAll:function(){this._aList=[]
},stopAll:function(){for(var b=0,a=this._aList.length;b<a;b++){this._aList[b].stop()}},run:function(c,b){for(var a=this._aList.length-1;
a>=0;a--){if(this._aList[a]){if(this._aList[a].isPlaying()){this._aList[a].run(c,b)}else{this._aList.splice(a,1)}}}}});collie.Timer=collie.Timer||new (collie.Class({$init:function(){this._oList=new collie.TimerList()
},run:function(b,a){this._oList.run(b,a)},stopAll:function(){this._oList.stopAll()},removeAll:function(){this._oList.removeAll()
},queue:function(c,a){var b=new collie.AnimationQueue(c,a);b.setTimerList(this._oList);return b},repeat:function(b,d,a){var c=new collie.AnimationRepeat(b,d,a);
...
}else{collie.util.addEventListener(window,"focus",this._onPageShow.bind(this));collie.util.addEventListener(window,"blur",this._onPageHide.bind(this))
...
if(a=="hidden"){this.pause()}else{if(a=="visible"){this.resume()}}},refresh:function(){if(this._elParent!==null){var e=this._elParent.ownerDocument||this._elParent.document||document;
var a=e.documentElement;var c=e.body;if("getBoundingClientRect" in this._elParent){var b=this._elParent.getBoundingClientRect();
this._htPosition.x=b.left;this._htPosition.x+=a.scrollLeft||c.scrollLeft;this._htPosition.y=b.top;this._htPosition.y+=a.scrollTop||c.scrollTop;
this._htPosition.width=b.width;this._htPosition.height=b.height}else{this._htPosition.x=0;this._htPosition.y=0;this._htPosition.width=this._elParent.offsetWidth;
this._htPosition.height=this._elParent.offsetHeight;for(var d=this._elParent;d;d=d.offsetParent){this._htPosition.x+=d.offsetLeft;
this._htPosition.y+=d.offsetTop}for(var d=this._elParent.parentNode;d;d=d.parentNode){if(d.tagName=="BODY"){break}if(d.tagName=="TR"){this._htPosition.y+=2
}this._htPosition.x-=d.scrollLeft;this._htPosition.y-=d.scrollTop}}}},getPosition:function(){return this._bLoaded?this._htPosition:false
},addLayer:function(b){if(!b||!("type" in b)||b.type!="layer"){throw new Error("oLayer is not Layer instnace")}for(var c=0,a=this._aLayerList.length;
c<a;c++){if(this._aLayerList[c]==b){return}}this._aLayerList.push(b);if(this._bLoaded){b.load(this._elContainer,this._aLayerList.length);
...
if(!this._sRenderingMode||this._sRenderingMode=="auto"){if((a.android&&(a.android>=3||a.android<2.2))||!a.supportCanvas||(a.ios&&a.ios<5)){this._sRenderingMode="dom"
...
this._sRenderingMode=null},isRetinaDisplay:function(){if(this._bUseRetinaDisplay===null){this._bUseRetinaDisplay=this.RETINA_DISPLAY!="auto"?this.RETINA_DISPLAY:window.devicePixelRatio>=2&&!collie.util.getDeviceInfo().android;
